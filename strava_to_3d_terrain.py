...
def carve_route_trench(x2d, y2d, elev_grid,
                       route_x, route_y,
                       trench_width_mm,
                       depth_m=5):

    trench_radius = trench_width_mm * 0.75

    for rx, ry in zip(route_x, route_y):
        dist = np.sqrt((x2d - rx)**2 + (y2d - ry)**2)

        mask = dist < trench_radius
        elev_grid[mask] -= depth_m * (1 - dist[mask] / trench_radius)
